...
> 当前默认补充（2026-09-12）：源码现已将文本/规划默认模型改为 MiniCPM5-2B
> Q4_K_M，并纳入 v0.4.1。本文的 1B 叙述与测试数据保留为各历史阶段的记录；
> 模型更换的本机测量发生在正式安装包构建之前。当前行为见[架构文档](architecture.md)，
> 模型更换的证据与边界见[本机对照记录](local-model-benchmark-2026-09-12.md)。
...
如果只关心当前实现，请先阅读[架构文档](architecture.md)；本文第 5～10 节保留
历史实现的设计理由。如果要理解为什么没有直接采用
