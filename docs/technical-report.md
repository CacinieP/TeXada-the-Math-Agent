# TeXada 技术报告：v0.3.2 基线与当前模型更新

> 报告基线：v0.3.2
> 基线日期：2026-07-28
> 定位：基于端侧 Agent 的结构化数学编辑器
> 当前源码默认（2026-09-12）：MiniCPM5-2B Q4_K_M（规划与文本）+ MiniCPM-V 4.6（视觉 OCR）

> 历史更新（2026-08-05）：本报告内容基线为 v0.3.2。当时 `main` 与桌面发布版本为
...
当前文本模型默认值为 `hf.co/openbmb/MiniCPM5-2B-GGUF:Q4_K_M`，视觉模型仍为
`openbmb/minicpm-v4.6:latest`。已保存的模型设置不会因源码默认值改变而自动迁移。
Ollama 的 MiniCPM5-2B 文本请求通过 `extra_body` 发送 `reasoning_effort: "none"`，
覆盖规划、生成、补全及重试；OCR、其他模型和 OpenAI-compatible 后端保留原行为。
现行状态与提交边界见[架构文档](architecture.md)。

这些模型与兼容修复纳入 v0.4.1；下面引用的本机测量发生在正式安装包构建之前。
[本地模型对照记录](local-model-benchmark-2026-09-12.md)说明了测试范围、结果与关闭
思考的质量取舍。六个开发回归用例不能代表通用数学准确率。

以下章节保留 v0.3.2 及所注明历史阶段的 1B 模型选型、测试计数和测量事实；不要将
这些数值当作当前 2B 默认模型的性能数据。
