# TeXada v0.1 技术报告（历史归档）

> 本文保留早期模型选型与实现描述，不是当前配置指南。2026-09-12 的源码文本默认
> 已改为 MiniCPM5-2B Q4_K_M；见[当前架构](architecture.md)与
> [本机模型对照记录](local-model-benchmark-2026-09-12.md)。下文 1B 数据不作回写。
...
| 中文术语翻译 | 字典替换（按长度优先） | ❌ |
| 快捷公式查找 | JSON 键值查询 | ❌ |
| LaTeX 语法验证 | 内容门禁 + 括号/环境 + 内存 KaTeX | ❌ |
| 自动修复 | 规则引擎（补全括号等） | ❌ |
| 渲染 | 内存 KaTeX V8 / 语法高亮 | ❌ |
...
- **视觉 OCR**：MiniCPM-V 4.6 on Ollama `/v1`，处理图片输入
- **兼容传输**：OpenAI-compatible endpoint 只作为同一组 MiniCPM 模型的部署方式，
  设置页填写 endpoint / model / vision model / API key
